...
describe("Agent improvement Git candidate", () => {
  test("runs the approved improve path through isolated authoring and matched Eval receipts", async () => {
    ...
    const result = await authorAgentImprovementCandidate(
      run,
      {
        ...
        gradeTaskAttempt: async () => {
          throw new Error("Injected receipt normalizer handles this fixture.");
        },
      },
      {
        ...
        runChecks: async () => ({
          checkRefs: [
            "profiles/default/agents/support/.openpond/agent-inspect.json",
          ],
    ...
    expect(result.candidates[0]).toMatchObject({
      ...
      metadata: {
        agentPackageLineage: {
          baseVersionId: `agent-${PACKAGE_DIGEST.slice(0, 20)}`,
          basePackageDigest: PACKAGE_DIGEST,
          validationReceiptIds: ["validation_fixture"],
          evalReceiptIds: ["eval_fixture"],
          candidateBaseCommit: fixture.head,
        },
      },
    });
    expect(result.evaluationReceipts).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ subject: "active", status: "failed" }),
        expect.objectContaining({ subject: "candidate", status: "passed" }),
      ])
    );
    expect(result.externalExecutionRefs).toEqual(
      expect.arrayContaining([
  ...
  test("snapshots dirty Profile source into the candidate baseline without requiring a remote", async () => {
    ...
    await writeFile(
      fixture.agentSource,
      "export const behavior = 'working tree';\n",
      "utf8"
    );
    ...
    const privateTasksetPath = path.join(
      fixture.sourcePath,
      "tasksets",
      "private",
      "fixtures.json"
  ...
  test("merges the reviewed candidate into the dirty local Profile without requiring GitHub", async () => {
    ...
    expect(
      (await git(fixture.repoPath, ["status", "--short"])).stdout.trim()
    ).toBe("");
    expect(
      (
  ...
  test("blocks changes outside the approved Agent paths", async () => {
    ...
    const workspace = await prepareAgentImprovementWorkspace({ run, target });
    await writeFile(
      path.join(workspace.target.repoPath, "README.md"),
      "outside\n",
      "utf8"
    );
  ...
  test("pushes one candidate branch and normalizes the created GitHub PR", async () => {
    ...
    expect(
      calls.filter((call) => call[0] === "git" && call[1] === "push")
    ).toHaveLength(1);
    expect(
...
async function profileRepoFixture() {
  const repoPath = await mkdtemp(
    path.join(os.tmpdir(), "openpond-agent-improvement-")
  );
  ...
  await mkdir(path.join(sourceRoot, ".openpond"), { recursive: true });
  await mkdir(path.join(sourcePath, "settings"), { recursive: true });
  await writeFile(agentSource, "export const behavior = 'active';\n", "utf8");
  await writeFile(
    path.join(sourceRoot, ".openpond", "agent-package-origin.json"),
    `${JSON.stringify(
      {
        schema: "openpond.agent-package-origin.v1",
        versionId: `agent-${PACKAGE_DIGEST.slice(0, 20)}`,
  ...
  await git(repoPath, [
    ...
    "add",
    "origin",
    "git@github.com:openpond/profile.git",
  ]);
