...
describe("chat work trace presentation", () => {
  test("hides reasoning and tool rows while the trace is closed", () => {
    const live = workTracePresentation(activities(84), false);
    expect(live.toolsExpanded).toBe(false);
    expect(live.toolCount).toBe(42);
    expect(live.visibleActivities).toEqual([]);
  });

  test("restores only tool rows in their original transcript order when opened", () => {
    const collapsed = workTracePresentation(activities(6), false);
    expect(collapsed.visibleActivities).toEqual([]);
  ...
  test("formats stable completion durations", () => {
    expect(
      formatWorkTraceDuration(
        ...
        "2026-07-22T15:01:24.000Z"
      )
    ).toBe("1m 24s");
    expect(
      formatWorkTraceDuration(
        "2026-07-22T15:00:00.000Z",
        "2026-07-22T15:00:00.400Z"
      )
    ).toBeNull();
  });
  ...
  test("maps common shell commands to readable activity verbs", () => {
    ...
    expect(summarizeShellCommand("rg --files apps/web/src")).toBe(
      "Listed files matching apps/web/src"
    );
    expect(summarizeShellCommand("ls -la apps/web/src")).toBe(
      "Listed files in apps/web/src"
    );
    expect(summarizeShellCommand("sed -n '1,120p' app.ts")).toBe(
      "Read lines 1-120 of app.ts"
    );
    expect(
      summarizeShellCommand("pnpm exec vitest run tests/chat-messages.test.ts")
    ).toBe("Ran tests");
    expect(summarizeShellCommand("git diff --check")).toBe("Reviewed changes");
    expect(summarizeShellCommand("./cli --staging frontend deploy")).toBe(
      "Ran cli command"
    );
    expect(summarizeShellCommand("write_stdin")).toBe("Continued command");
    expect(summarizeShellCommand("js")).toBe("Ran JavaScript");
    expect(
      summarizeShellCommand("rg activity-summary apps/web/src", "running")
