...
import { executeAgentImprovementReleaseAction } from "../apps/server/src/runtime/create-pipeline/agent-improvement-release";
...
describe("Agent improvement PR release", () => {
  ...
  test("opens one PR, reconciles its merge, syncs the Profile, and requires post-merge Eval success", async () => {
    ...
    expect(opened.externalExecutionRefs).toContainEqual(
    ...
    expect(released.candidates[0]).toMatchObject({
      ...
      metadata: {
        agentPackageLineage: {
          baseVersionId: "agent-base",
          basePackageDigest: "d".repeat(64),
          releasedSourceCommit: "c".repeat(40),
          postReleaseReceiptId: "post_release_eval",
        },
      },
    });
    expect(released.evaluationReceipts).toContainEqual(
      expect.objectContaining({
        subject: "post_release",
        status: "passed",
      })
    );
    expect(released.evidenceSnapshots).toContainEqual(
      expect.objectContaining({
        reviewerIntent:
