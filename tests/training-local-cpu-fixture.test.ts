...
async function setupFixture(store: any, directory: string, options: { customVerifier?: boolean } = {}) {
  ...
  const destination = new LocalCpuTrainingDestination({ store, storeDir: directory, projectDir: path.resolve("python/openpond-training"), loadProfileState: async () => ({ mode: "local", activeProfile: "default", sourcePath: profileSource } as any) });
