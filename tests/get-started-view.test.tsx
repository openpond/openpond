...
describe("GetStartedView", () => {
  test("orders the Agent overview, walkthroughs, deeper learning, and guides", () => {
    ...
    expect(seriesHtml).toContain(
      "Open Post-training from first principles playlist"
    );
    ...
    expect(pageHtml).toContain(
      'id="get-started-walkthroughs-title">Walkthroughs</h2>'
    );
    expect(pageHtml).toContain(
      'id="get-started-learn-title">Learn deeper</h2>'
    );
    ...
    expect(pageHtml.indexOf(OPENPOND_AGENT_OVERVIEW.title)).toBeLessThan(
      pageHtml.indexOf("Agents")
    );
    expect(pageHtml.indexOf("Agents")).toBeLessThan(
      pageHtml.indexOf("Post-training from first principles")
    );
    expect(
      pageHtml.indexOf("Post-training from first principles")
    ).toBeLessThan(pageHtml.indexOf("How OpenPond works"));
  });
  ...
  test("opens each lesson's named Markdown script from its right-panel row", () => {
    ...
    expect(lessonsHtml).toContain(
      "workspace-diff-panel get-started-learning-panel"
    );
    ...
    expect(lessonsHtml).toContain(
      "Open script_01.md for How post-training works"
    );
    ...
    expect(lessonsHtml.match(/get-started-learning-lesson-card/g)).toHaveLength(
      10
    );
    expect(lessonsHtml).toContain("Full video");
    expect(lessonsHtml).toContain("29:20 · All 10 lessons");
    expect(lessonsHtml).toContain(
      `Play full video: ${POST_TRAINING_FULL_COURSE.title}`
    );
  ...
  test("keeps the learning series ordered and individually loadable", () => {
    ...
    expect(
      POST_TRAINING_LESSONS.every((lesson) => lesson.videoUrl.endsWith(".mp4"))
    ).toBe(true);
    expect(
      POST_TRAINING_LESSONS.every((lesson) =>
        lesson.captionsUrl?.endsWith(".vtt")
      )
    ).toBe(true);
    expect(
      POST_TRAINING_LESSONS.map((lesson) => lesson.script?.fileName)
    ).toEqual([
      "script_01.md",
      "script_02.md",
      "script_03.md",
    ...
    expect(
      POST_TRAINING_LESSONS.every(
        (lesson) =>
          lesson.script?.url.includes("/scripts/script_") &&
          lesson.script.url.endsWith(".md")
      )
    ).toBe(true);
  ...
  test("normalizes, reports, resumes, and sequences persistent lesson progress", () => {
    ...
    expect(
      postTrainingProgressPercent(progress[POST_TRAINING_LESSONS[0]!.id])
    ).toBe(100);
    expect(
      postTrainingProgressPercent(progress[POST_TRAINING_LESSONS[1]!.id])
    ).toBe(50);
    expect(postTrainingResumeTime(progress[POST_TRAINING_LESSONS[1]!.id])).toBe(
      31
    );
    expect(postTrainingResumeTime(progress[POST_TRAINING_LESSONS[0]!.id])).toBe(
      0
    );
    expect(
      startingPostTrainingLessonIndex(
  ...
  test("renders the learning page with the Goal loop guide selected by default", () => {
    ...
    expect(html).not.toContain("Train and deploy with OpenPond");
    ...
    expect(html).not.toContain(
      "Learn how goals, source-backed agents, hosted profiles, and connected apps work together."
    );
    expect(html).not.toContain(
      "How local and hosted goals keep objective, evidence, and completion state visible."
    );
  ...
  test("puts the Agents playlist and its full video footer in Walkthroughs", () => {
    const walkthroughCardHtml = renderToStaticMarkup(
      createElement(MakeAgentTutorialCard)
    );
    ...
    expect(walkthroughCardHtml).toContain(
      "how-to-make-an-agent-create-poster.png"
    );
    expect(walkthroughCardHtml).toContain(
      "how-to-make-an-agent-use-poster.png"
    );
    expect(walkthroughCardHtml).toContain(
      "how-to-make-an-agent-improve-poster.png"
    );
    ...
    expect(walkthroughPlayerHtml).toContain(
      "tutorials/how-to-make-an-agent.mp4"
    );
    expect(walkthroughPlayerHtml).toContain(
  ...
  test("gives the Make Agent walkthrough the same lessons and script sidebar pattern", () => {
    const lessonsHtml = renderToStaticMarkup(
      createElement(MakeAgentTutorialLearningPanel, {
    ...
    expect(makeAgentTutorialScript("improve")).not.toContain(
      "Start from a prompt or chats"
    );
  });
  ...
  test("keeps local-hosted copy explicit about profile and target mounts", () => {
    ...
    const mountSlide = deck?.slides.find(
      (slide) => slide.id === "local-hosted-mounts"
    );
    ...
    const deckHtml = renderToStaticMarkup(
      createElement(GetStartedDeckView, { deck: deck! })
    );
    const mountHtml = renderToStaticMarkup(
      createElement(GetStartedVisual, { accent: "cyan", kind: "dual-source" })
    );
  ...
  test("keeps local-hosted publish copy focused on reviewed snapshots", () => {
    const localHostedDeck = GET_STARTED_DECKS.find(
      (item) => item.id === "local-hosted"
    );
    const publishSlide = localHostedDeck?.slides.find(
      (slide) => slide.id === "local-hosted-publish"
    );
  ...
  test("keeps slide accents and right-side diagrams restrained", () => {
    const accents = new Set(
      GET_STARTED_DECKS.flatMap((deck) =>
        deck.slides.map((slide) => slide.accent)
      )
    );
    const html = renderToStaticMarkup(
      createElement(GetStartedVisual, { accent: "cyan", kind: "dual-source" })
  ...
  test("keeps the conceptual guide decks", () => {
    ...
    expect(GET_STARTED_DECKS.some((deck) => deck.id === "train-deploy")).toBe(
      false
    );
  });
  ...
  test("frames create and edit as first-class goal-loop agents", () => {
    ...
    expect(buildDeck?.description).toContain(
      "specific implementations on top of goals"
    );
    expect(firstSlide?.eyebrow).toBe("First-class agents");
    expect(firstSlide?.title).toBe("Create/Edit is built on goals");
    expect(firstSlide?.body).toContain(
      "implementations on top of the goal loop"
    );
  ...
  test("adds an insights loop deck based on create/edit pipeline state", () => {
    const insightsDeck = GET_STARTED_DECKS.find(
      (deck) => deck.id === "insights"
    );
  ...
  test("renames profile deck and focuses it on a portable Git-backed repository", () => {
    ...
    expect(profileDeck?.slides[0]?.title).toBe(
      "One agent repository travels with you"
    );
    expect(profileDeck?.slides[0]?.detail).toContain("Git-backed");
    expect(profileDeck?.slides[1]?.title).toBe(
      "Artifacts make agents portable"
    );
  });

  test("uses real connected app icons and keeps app slides title-led", () => {
    const connectDeck = GET_STARTED_DECKS.find(
      (deck) => deck.id === "connect-apps"
    );
    const appsSlide = connectDeck?.slides.find(
      (slide) => slide.id === "connect-apps-grid"
    );
    const channelSlide = connectDeck?.slides.find(
      (slide) => slide.id === "connect-channels"
    );
    const appsVisualHtml = renderToStaticMarkup(
      createElement(GetStartedVisual, { accent: "cyan", kind: "apps-grid" })
    );
    const channelVisualHtml = renderToStaticMarkup(
      createElement(GetStartedVisual, {
        accent: "cyan",
        kind: "channel-router",
      })
    );
