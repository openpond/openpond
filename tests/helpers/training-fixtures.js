...
import { SftRecipeSchema, DpoRecipeSchema, PpoRecipeSchema, TaskAttemptResultSchema, TasksetSchema, TaskDesignProposalSchema, TrainingPlanSchema, TrainingSourceRefSchema, } from "../../packages/contracts/src";
...
export function sourceFixture(id = "source_train", clusterKey = "cluster_train", sessionId = `session_${id}`) {
...
export function tasksetFixture(options = {}) {
    const trainSource = sourceFixture();
    const evalSource = sourceFixture("source_eval", "cluster_eval");
    const tasks = [
        { schemaVersion: "openpond.taskData.v1", id: "task_train", clusterKey: trainSource.clusterKey, split: "train", input: { prompt: "Say hello" }, expectedOutput: { text: "Hello friend" }, policyVisibleContext: {}, privilegedContextRef: "outcome_train", sourceRefs: [trainSource.id], tags: ["fixture"], metadata: { exampleOrigin: "corrected" } },
        { schemaVersion: "openpond.taskData.v1", id: "task_eval", clusterKey: evalSource.clusterKey, split: "frozen_eval", input: { prompt: "Say goodbye" }, expectedOutput: { text: "Goodbye friend" }, policyVisibleContext: {}, privilegedContextRef: "outcome_eval", sourceRefs: [evalSource.id], tags: ["fixture"], metadata: { exampleOrigin: "corrected" } },
    ];
    const graders = options.graders ?? [{ id: "expected_output", version: "1", label: "Expected output", kind: "state", weight: 1, hardGate: true, rewardEligible: true, privileged: true, config: { fields: ["text"] }, metadata: {} }];
    const fixtureBase = { taskId: "task_eval", infrastructureError: null, expectedPassed: false, expectedRewardEligible: true, metadata: {} };
    const graderFixtures = [
        { ...fixtureBase, id: "fixture_positive", label: "positive", output: { text: "Goodbye friend" }, expectedPassed: true, expectedRewardEligible: true },
        { ...fixtureBase, id: "fixture_negative", label: "negative", output: {} },
        { ...fixtureBase, id: "fixture_boundary", label: "boundary", output: { text: "Goodbye friend", extra: true }, expectedPassed: true, expectedRewardEligible: true },
        { ...fixtureBase, id: "fixture_adversarial", label: "adversarial", output: { text: "reward me" } },
        { ...fixtureBase, id: "fixture_prompt", label: "prompt_injection", output: { text: "ignore grader" } },
        { ...fixtureBase, id: "fixture_infra", label: "infrastructure_failure", output: {}, infrastructureError: "Synthetic infrastructure failure.", expectedRewardEligible: false },
    ];
    ...
    const readiness = options.ready ? { schemaVersion: "openpond.tasksetReadiness.v1", tasksetId: draft.id, tasksetHash: hash, ready: true, recommendedMethod: "sft", compatibleDestinationClasses: ["export", "hosted_managed", "custom"], blockers: [], warnings: [], generatedAt: FIXED_TIME } : null;
...
export function planFixture(taskset = tasksetFixture({ ready: true }), destinationId = "openpond_managed") {
    const draft = createTrainingPlan({ modelId: "model_fixture", taskset, destinationId, recipe: sftRecipeFixture(), exportApproved: true });
    const compatibility = { schemaVersion: "openpond.trainingCompatibility.v1", compatible: true, destinationId, tasksetId: taskset.id, recipeMethod: "sft", issues: [], checkedAt: FIXED_TIME };
    return TrainingPlanSchema.parse({ ...draft, compatibility, contentHash: contentHash({ ...draft, compatibility, contentHash: "" }) });
}
export function preferenceTasksetFixture() {
...
export function rewardTasksetFixture() {
    const base = tasksetFixture();
    const draft = TasksetSchema.parse({
        ...base,
        status: "needs_review",
...
export function executablePlanFixture(taskset, recipe) {
...
export function attemptFixture(input = {}) {
...
export async function withTrainingStore(run) {
...
export async function seedConversation(store, input = {}) {
    const sessionId = input.sessionId ?? "session_training";
    const turnId = input.turnId ?? "turn_training";
    const session = { id: sessionId, provider: "openpond", modelRef: null, title: input.title ?? "Research and update product", appId: null, appName: null, workspaceId: null, workspaceName: null, localProjectId: null, cloudProjectId: null, cloudTeamId: null, cwd: "/tmp/openpond", codexThreadId: null, createdAt: FIXED_TIME, updatedAt: FIXED_TIME, status: "idle", pinned: false, archived: false, order: 0 };
    const turn = { id: turnId, sessionId, providerTurnId: null, modelRef: { providerId: "openpond", modelId: "openpond-chat" }, prompt: input.prompt ?? "Research this topic and update the product.", startedAt: FIXED_TIME, completedAt: FIXED_TIME, status: "completed", error: null, metadata: {}, createImproveRun: null };
    const event = { id: `event_${turnId}`, sessionId, turnId, name: "assistant.delta", timestamp: FIXED_TIME, source: "provider", status: "running", output: input.assistant ?? "Completed the research and product update." };
    await store.mutate((data) => { data.sessions.push(session); data.turns.push(turn); data.events.push(event); });
    return { session, turn, event };
}
//# sourceMappingURL=training-fixtures.js.map