...
export function tasksetFixture(options: { ready?: boolean; profileId?: string; graders?: Taskset["graders"] } = {}): Taskset {
  ...
  const graderFixtures = [
    ...
    { ...fixtureBase, id: "fixture_infra", label: "infrastructure_failure" as const, output: {}, infrastructureError: "Synthetic infrastructure failure.", expectedRewardEligible: false },
  ];
...
export async function seedConversation(store: SqliteStore, input: { sessionId?: string; turnId?: string; title?: string; prompt?: string; assistant?: string } = {}) {
  ...
  const session: Session = SessionSchema.parse({ id: sessionId, provider: "openpond", modelRef: null, title: input.title ?? "Research and update product", appId: null, appName: null, workspaceId: null, workspaceName: null, localProjectId: null, cloudProjectId: null, cloudTeamId: null, cwd: "/tmp/openpond", codexThreadId: null, createdAt: FIXED_TIME, updatedAt: FIXED_TIME, status: "idle", pinned: false, archived: false, order: 0 });
  ...
  const event: RuntimeEvent = { id: `event_${turnId}`, sessionId, turnId, name: "assistant.delta", timestamp: FIXED_TIME, source: "provider", status: "started", output: input.assistant ?? "Completed the research and product update." };
