import type { HostedSavedWorkSchedule, LocalAgentSchedule } from "@openpond/contracts";
...
describe("scheduled work projections", () => {
  ...
  test("formats rate and common cron cadences", () => {
    ...
    expect(localScheduleCadence(schedule("weekday", {
      scheduleType: "cron",
      scheduleExpression: "0 9 * * MON-FRI",
      timezone: "America/New_York",
    }))).toBe("Weekdays at 9:00 AM · America/New_York");
    expect(scheduleCadence(hostedSchedule())).toBe("Daily at 8:00 AM · America/New_York");
    expect(formatScheduledRunAt("not-a-date", "UTC")).toBe("None");
  });

  test("builds external hosted conversation links", () => {
    expect(hostedConversationUrl("https://openpond.ai", "conversation/1"))
      .toBe("https://openpond.ai/sandboxes/work/conversation%2F1");
    expect(hostedConversationUrl("https://openpond.ai", null))
      .toBe("https://openpond.ai/sandboxes/work");
  });
});

function schedule(id: string, input: Partial<LocalAgentSchedule> = {}): LocalAgentSchedule {
  return {
    id,
