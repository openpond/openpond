...
export async function queueRewardCheck(tx: LearningTransaction, input: Extract<LearningCommand, { action: "queue_reward_check" }>, operationId: string, now: string) {
  ...
  const check = RewardCheckRunSchema.parse({
    ...
    timeoutMs: input.timeoutMs, maximumSpendUsd: input.maximumSpendUsd,
