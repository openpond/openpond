import { executeJavaScriptIsolate, JavaScriptIsolateExecutionError, type JavaScriptIsolateInput } from "./javascript-isolate.js";
...
void executeJavaScriptIsolate(input).then(
  ...
  (error: unknown) => process.stdout.write(JSON.stringify({ ok: false, executionError: error instanceof JavaScriptIsolateExecutionError, error: (error instanceof Error ? error.message : "JavaScript execution failed.").slice(0, 8_192) })),
);
