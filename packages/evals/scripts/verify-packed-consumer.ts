...
const environmentDefinition = sealLearningContent({ schemaVersion: "openpond.javascriptEnvironment.v1", id: "packed-environment-module", revision: 1, module: environmentAsset.asset, tools: [{ name: "run", description: "Run a bounded step", inputSchema: toolSchema, inputSchemaHash: contentHash(toolSchema), sideEffect: "write", timeoutMs: 5000 }], maxSteps: 4, maxStateBytes: 4096, maxObservationBytes: 1024, operationTimeoutMs: 5000 });
const session = await createJavaScriptEnvironmentSession({ definition: environmentDefinition, asset: environmentAsset, input: {}, initialState: { value: 1 }, seed: 0, execute: executeJavaScriptEnvironmentInWorker });
const runningStep = session.step({ name: "run", arguments: {} }).then(() => { throw new Error("Cancelled environment step succeeded"); }, error => { if (error.message !== "environment_destroyed") throw error; });
