...
export const ModelUsageAttributionSchema = z.object({
  ...
  createImproveRunId: z.string().trim().min(1).nullable().default(null),
  commandName: z.string().trim().min(1).max(160).nullable().default(null),
  commandSource: UsageCommandSourceSchema.nullable().default(null),
  appId: z.string().trim().min(1).nullable().default(null),
  workspaceKind: WorkspaceKindSchema.nullable().default(null),
  workspaceId: z.string().trim().min(1).nullable().default(null),
  localProjectId: z.string().trim().min(1).nullable().default(null),
  cloudProjectId: z.string().trim().min(1).nullable().default(null),
  sourceEventSequence: z.number().int().nonnegative().nullable().default(null),
});
...
export const UsageRequestAttributionSchema = z.object({
  ...
  createImproveRunId: z.string().trim().min(1).nullable().optional(),
