...
export function createTrainingPlan(input: {
  ...
  const environmentPlacement = input.destinationId === "openpond_managed"
    ...
    : ("local" as const);
  const id = `training_plan_${contentHash([input.modelId, input.taskset.contentHash, input.destinationId, environmentPlacement, input.recipe]).slice(0, 24)}`;
