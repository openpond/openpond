...
export function createTrainingPlan(input: {
  ...
  const id = `training_plan_${contentHash([input.taskset.contentHash, input.destinationId, input.recipe]).slice(0, 24)}`;
