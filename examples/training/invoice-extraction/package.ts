...
export function createInvoiceStarterPackage() {
  ...
  const tasks = invoiceTasks();
  const trainingTasks = tasks.filter(task => task.split === "train");
  const graderFixtures = trainingTasks.flatMap(task => {
    if (!task.expectedOutput) throw new Error("Invoice authoring requires a reference target.");
    return [
      { id: `correct-${task.id}`, taskId: task.id, label: "positive", output: task.expectedOutput, infrastructureError: null, expectedPassed: true, expectedRewardEligible: true, metadata: { origin: "original_synthetic_reference" } },
      { id: `incorrect-${task.id}`, taskId: task.id, label: "negative", output: { ...task.expectedOutput, invoiceNumber: "WRONG" }, infrastructureError: null, expectedPassed: false, expectedRewardEligible: true, metadata: { origin: "authored_identifier_corruption" } },
    ];
  });
  const taskset = TasksetReleaseSchema.parse(sealLearningContent({ schemaVersion: "openpond.tasksetRelease.v2", id: "starter-invoice-tasks", revision: 1, ...definition.execution, tasks, graders: compileBoundGraders(binding, [reward]), metadata: {
    starter: { taskDefinition: learningRef(definition), rewardBinding: learningRef(binding) },
    starterAuthoring: { schemaVersion: "openpond.starterAuthoring.v1", publishedAt: "2026-09-06T20:00:00.000Z", reviewedBy: "OpenPond synthetic invoice authoring pipeline", licensingStatus: "approved", approvedTrainingTaskIds: trainingTasks.map(task => task.id), graderFixtures },
    provenance: { author: "OpenPond", source: "Original deterministic synthetic invoice documents", license: "MIT" },
  } }));
