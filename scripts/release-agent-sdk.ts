...
const packageFile = path.join(root, "packages/agent-sdk/package.json");
...
function nextVersion(current: string, kind: ReleaseKind): string {
  ...
  if (!match) throw new Error(`Agent SDK version must be stable semver, got ${current}`);
...
async function main(): Promise<void> {
  ...
  if (!kind) throw new Error("Usage: pnpm release:agent-sdk:<patch|minor|major> [--dry-run]");
  const dryRun = args.includes("--dry-run");
  if (output("git", ["status", "--porcelain"])) throw new Error("Working tree must be clean before preparing an Agent SDK release.");
  const branch = output("git", ["branch", "--show-current"]);
  if (branch !== "master") throw new Error(`Agent SDK releases must start on master, got ${branch}.`);
  ...
  const releaseBranch = `release/agent-sdk-v${version}`;
  const title = `chore(agent-sdk): release v${version}`;
  ...
  else console.log(`[dry-run] set packages/agent-sdk/package.json version to ${version}`);
  run("pnpm", ["install", "--lockfile-only", "--strict-peer-dependencies=false"], dryRun);
  run("pnpm", ["run", "agent-sdk:check"], dryRun);
  run("git", ["add", "packages/agent-sdk/package.json", "pnpm-lock.yaml"], dryRun);
  run("git", ["diff", "--cached", "--check"], dryRun);
  run("git", ["commit", "-m", title], dryRun);
  run("git", ["push", "--set-upstream", "origin", releaseBranch], dryRun);
  run("gh", ["pr", "create", "--base", "master", "--head", releaseBranch, "--title", title, "--body", `Publishes \`openpond-agent-sdk@${version}\` after this PR merges and CI passes.`], dryRun);
  console.log(`Merge the release PR after CI passes. release-agent-sdk.yml will publish openpond-agent-sdk@${version}.`);
}
