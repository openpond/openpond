...
const packageFile = path.join(root, "packages/evals/package.json");
...
function nextVersion(current: string, kind: ReleaseKind): string {
  ...
  if (!match) throw new Error(`Evals version must be stable semver, got ${current}`);
...
async function main(): Promise<void> {
  ...
  if (!kind) throw new Error("Usage: pnpm release:evals:<patch|minor|major> [--dry-run]");
  const dryRun = args.includes("--dry-run");
  if (output("git", ["status", "--porcelain"])) throw new Error("Working tree must be clean before preparing an Evals release.");
  const branch = output("git", ["branch", "--show-current"]);
  if (branch !== "master") throw new Error(`Evals releases must start on master, got ${branch}.`);
  ...
  const releaseBranch = `release/evals-v${version}`;
  const title = `chore(evals): release v${version}`;
  ...
  else console.log(`[dry-run] set packages/evals/package.json version to ${version}`);
  run("pnpm", ["install", "--lockfile-only", "--strict-peer-dependencies=false"], dryRun);
  run("pnpm", ["run", "evals:check"], dryRun);
  run("git", ["add", "packages/evals/package.json", "pnpm-lock.yaml"], dryRun);
  run("git", ["diff", "--cached", "--check"], dryRun);
  run("git", ["commit", "-m", title], dryRun);
  run("git", ["push", "--set-upstream", "origin", releaseBranch], dryRun);
  run("gh", ["pr", "create", "--base", "master", "--head", releaseBranch, "--title", title, "--body", `Publishes \`@openpond/evals@${version}\` after this PR merges and CI passes. This does not change the desktop, CLI, TUI, or server SDK version.`], dryRun);
  console.log(`Merge the release PR after CI passes. release-evals.yml will publish @openpond/evals@${version}.`);
}
