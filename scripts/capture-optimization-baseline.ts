...
function parseArgs(args: string[], root: string): BaselineOptions {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === "--help" || arg === "-h") {
      console.log([
        "usage: pnpm exec tsx scripts/capture-optimization-baseline.ts [options]",
        "",
        "Options:",
...
function renderMarkdownBaseline(
  ...
  return [
    ...
    "",
    `Generated by \`pnpm baseline:optimization\` on ${baseline.platform.os}/${baseline.platform.arch}.`,
    "",
    "## Summary",
    "",
    "| Area | Metric | Result |",
