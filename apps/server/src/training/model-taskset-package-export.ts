...
import { captureAuthoredModelTasksetPackage } from "./model-taskset-package-capture.js";
...
export async function exportLocalModelTasksetPackage(input: {
  ...
  const captured = await captureAuthoredModelTasksetPackage({ storeDir: input.storeDir, taskset,
    content: { schemaVersion: "openpond.tasksetPackage.v1", taskset: release,
      environment: releases.environmentRelease, verifierSet: releases.verifierSetRelease,
      ...(modelResources ? { modelResources } : {}), ...(learningResources ? { learningResources } : {}) },
  });
