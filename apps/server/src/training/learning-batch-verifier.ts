...
export function createLearningBatchVerifier(store: SqliteStore, taskset: Taskset, storeDir?: string): CustomVerifierRunner {
  ...
  return async ({ grader, task, attempt, signal }) => {
    ...
    if (!evidence) throw new Error("Imported batch evidence is missing from its package.");
    ...
    if (imported) {
      const asset = imported.learningResources?.assets.find(asset => asset.id === bound.verifierRef.id);
      if (!asset) throw new Error("Imported batch verifier is missing from its package.");
      return executeJavaScriptVerifierInWorker({ source: verifyLearningTextAsset(asset, bound.verifierRef), exportName: bound.exportName,
        timeoutMs: bound.timeoutMs, signal, value: { task: releasedTask, attempt: { output: attempt.output,
          artifactRefs: attempt.artifactRefs, runtimeEventRefs: attempt.runtimeEventRefs, infrastructureError: attempt.infrastructureError },
          input: releasedTask.input, output: attempt.output, expectedOutput: releasedTask.expectedOutput,
          evaluatorContext: evidence.submission.evaluatorContext, infrastructureError: attempt.infrastructureError ?? null } });
    }
