...
export class LocalCpuTrainingDestination implements TrainingDestination {
  ...
  constructor(private readonly deps: { store: SqliteStore; storeDir: string; projectDir: string; loadProfileState?: typeof loadOpenPondProfileState }) {}
  ...
  async launch(plan: TrainingPlan, approval: TrainingApproval): Promise<TrainingJob> {
    ...
    const child = spawn("uv", ["run", "--project", this.deps.projectDir, "openpond-training", "run", "--bundle", bundleDirectory, "--output", outputDirectory, "--job-id", jobId, "--cancel-file", cancelFile, "--taskset", tasksetPath], {
  ...
  async importExternal(input: { planId: string; bundleId: string; artifactDirectory: string }): Promise<TrainingJob> {
    ...
    const bundleDirectory = path.join(this.deps.storeDir, "training", "bundles", plan.id);
    const cancelFile = path.join(outputDirectory, "cancel.requested");
    const child = spawn("uv", ["run", "--project", this.deps.projectDir, "openpond-training", "evaluate", "--bundle", bundleDirectory, "--output", outputDirectory, "--job-id", jobId, "--cancel-file", cancelFile, "--taskset", tasksetPath], { cwd: this.deps.projectDir, env: { ...process.env, PYTHONUNBUFFERED: "1", TOKENIZERS_PARALLELISM: "false" }, stdio: ["pipe", "pipe", "pipe"] });
    child.stdin.end();
  ...
  private async consume(initialJob: TrainingJob, child: ChildProcessWithoutNullStreams, outputDirectory: string): Promise<void> {
    ...
    await this.deps.store.saveTrainingJob({ ...latest, status: "succeeded", completedAt: timestamp, updatedAt: timestamp, error: null, metadata: { ...latest.metadata, artifactCount: artifacts.length, reloadVerified: true, frozenEvaluationExecuted: Boolean(evaluation) } });
  }
  ...
  private async importArtifacts(job: TrainingJob, outputDirectory: string): Promise<TrainingArtifact[]> {
    ...
    for (const item of manifest.artifacts) {
      ...
      const kind = item.path.endsWith("adapter_model.safetensors") ? "adapter" : item.path === "metrics.json" ? "metrics" : item.path === "frozen-eval-predictions.jsonl" ? "evaluation" : item.path === "events.jsonl" ? "log" : "checkpoint";
    ...
    await this.importFrozenEvaluation(job, outputDirectory);
    return imported;
  }

  private async importFrozenEvaluation(job: TrainingJob, outputDirectory: string): Promise<void> {
    ...
    if (!taskset) return;
    let content: string;
    try { content = await readFile(path.join(outputDirectory, "frozen-eval-predictions.jsonl"), "utf8"); } catch { return; }
    for (const [index, line] of content.split("\n").filter(Boolean).entries()) {
      ...
      if (!task) continue;
      const timestamp = new Date().toISOString();
      const attemptId = `attempt_${contentHash([job.id, prediction.taskId, index]).slice(0, 24)}`;
      const rawDirectory = path.join(outputDirectory, "frozen-eval-attempts");
