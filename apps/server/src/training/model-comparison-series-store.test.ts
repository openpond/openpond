  ...
  it("archives a terminal execution attempt before clearing its active linkage", async () => {
    ...
    try {
      const cancelled = ModelComparisonSeriesEntrySchema.parse({
        ...
        status: "cancelled",
        trainableRank: 1,
        enabledCumulativeRank: 17,
        trainableBlockId: "block-child-store-test",
        residualBlocks: [
          {
            id: "block-parent-store-test",
            branchOrdinal: 0,
            rank: 16,
            offsetStart: 0,
            offsetEnd: 16,
            optimizationRole: "frozen",
            artifactLineageId: "lineage-parent-store-test",
          },
          {
            id: "block-child-store-test",
            branchOrdinal: 1,
            rank: 1,
            offsetStart: 16,
            offsetEnd: 17,
