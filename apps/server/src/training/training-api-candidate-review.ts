...
import type { createHostedRunCandidateReviewService } from "./hosted-run-candidate-review.js";
...
export async function handleManagedCandidateReviewAction(action: Action, input: Record<string, unknown>, localService: ReturnType<typeof createManagedCandidateReviewService>, hostedService: ReturnType<typeof createHostedRunCandidateReviewService>) {
  const modelId = z.string().trim().min(1).max(191).parse(input.jobId ?? input.modelId);
  const service = input.jobId === undefined ? localService : hostedService;
