...
export const HOSTED_RESOURCE_TOOL_FALLBACK_PROTOCOL = [
  ...
  "Use resource_search first to find stable refs, then resource_read on returned refs. Use targeted path or identifier queries with limit 5-10; avoid repeated broad one-word searches unless the word is an exact component/function/file identifier. Workspace resource_search defaults to exact literal path/text search; pass filters.mode=\"path\" for file/path lookup and filters.mode=\"ranked\" for broad multi-term retrieval. Do not use broad legacy actions such as list_files, read_files, search_files, sandbox_read_file, or sandbox_search_files in this fallback mode.",
