...
export function createSubagentToolRuntime(deps: {
  ...
  const subagentWorkspaceTargetKeyForSession =
    deps.subagentWorkspaceTargetKeyForSession;
  const subagentWorkspaceTargetKeyFromRun =
    deps.subagentWorkspaceTargetKeyFromRun;
  const subagentChildTurnPermissions = deps.subagentChildTurnPermissions;
  const prepareSubagentWorkspaceIsolation =
    deps.prepareSubagentWorkspaceIsolation;
  ...
  async function startSubagentFromModelTool(
    ...
    if (context.session.subagentRunId) {
      throw new Error(
        "Child subagents cannot start additional subagents in this version."
      );
    }
    const deps = requireSubagentDeps();
    const preferences = await loadAppPreferences();
    if (!preferences.subagents.enabled)
      throw new Error("Subagents are disabled in settings.");
    const role = preferences.subagents.roles.find(
      (candidate) => candidate.id === input.roleId
    );
    if (!role?.enabled)
      throw new Error(`Subagent role ${input.roleId} is not enabled.`);
    const parentModelRef = context.model
      ? { providerId: context.provider, modelId: context.model }
      : null;
    const modelRef = role.modelRef ??
      preferences.subagents.defaultModelRef ??
      parentModelRef ??
      preferences.defaultChatModelRef ?? {
        providerId: preferences.defaultChatProvider,
        modelId: preferences.defaultChatModel,
      };
    const workspaceTargetKey = await subagentWorkspaceTargetKeyForSession(
      context.session
    );
    const writeCapable = role.toolPolicy !== "read_only";
    if (
      writeCapable &&
      workspaceWriteStartReservations.has(workspaceTargetKey)
    ) {
      throw new Error(
        `Another write-capable subagent is starting for ${workspaceTargetKey}.`
      );
    }
    ...
    try {
      ...
      if (providerLimit !== null) {
        const activeProviderRuns = activeRuns.filter(
          (run) => run.modelRef?.providerId === modelRef.providerId
        );
      ...
      if (workspaceTargetLimit !== null && writeCapable) {
        ...
        const activeWorkspaceRuns = allActiveRuns.filter(
          (run) =>
            run.toolPolicy !== "read_only" &&
            subagentWorkspaceTargetKeyFromRun(run) === workspaceTargetKey
        );
      ...
      const childSession = await deps.createSession({
        experience: context.session.experience,
        ...
        workspaceKind:
          childSessionWorkspace.workspaceKind ?? context.session.workspaceKind,
        workspaceId:
          childSessionWorkspace.workspaceId ?? context.session.workspaceId,
        workspaceName:
          childSessionWorkspace.workspaceName ?? context.session.workspaceName,
        localProjectId:
          childSessionWorkspace.localProjectId ??
          context.session.localProjectId,
        cloudProjectId:
          childSessionWorkspace.cloudProjectId ??
          context.session.cloudProjectId,
        cloudTeamId:
          childSessionWorkspace.cloudTeamId ?? context.session.cloudTeamId,
      ...
      const run = SubagentRunSchema.parse({
        ...
        report: isolationBlocker
          ? {
              summary:
                "Child could not start because write-capable isolation is unavailable.",
      ...
      if (writeCapable)
        workspaceWriteStartReservations.delete(workspaceTargetKey);
    }
  }
  ...
  async function statusSubagentsFromModelTool(
    ...
    const runs = input.runId
      ? [await deps.getRun(input.runId)].filter((run): run is SubagentRun =>
          Boolean(run)
        )
      : await deps.listRuns({
          parentSessionId: context.session.id,
    ...
    return {
      runs: runs.map((run) =>
        subagentToolResultFromRun(run, "Subagent status loaded.")
      ),
      nextStep:
        runs.length === 0
          ? "No matching subagent runs."
          : `Loaded ${runs.length} subagent run${
              runs.length === 1 ? "" : "s"
            }.`,
    };
  }
  ...
  async function cancelSubagentFromModelTool(
    ...
    if (run.status === "completed") {
      return subagentToolResultFromRun(
        run,
        "Subagent already completed; no cancellation was applied."
      );
    }
    ...
    let cleanupResult: Record<string, unknown> | null =
      input.cleanupWorkspace === false
  ...
  async function followupSubagentFromModelTool(
    ...
    assertSubagentRunAccessible(context.session, run);
    if (!run.childSessionId)
      throw new Error(`Subagent run ${run.id} has no child conversation.`);
    if (run.status === "cancelled") {
      throw new Error(
        `Subagent run ${run.id} is ${run.status} and cannot receive a follow-up.`
      );
    }
  ...
  async function runSubagentLifecycleAction(
    ...
    return {
      ...
      workspaceCleanup,
      sessionArchive,
      nextStep: subagentLifecycleActionNextStep(
        input.action,
        workspaceCleanup,
        sessionArchive
      ),
