...
export async function executeAgentImprovementReleaseAction(input: {
  ...
  try {
    if (input.action.type === "apply_candidate") {
      ...
      const released =
        receipt.status === "passed" && receipt.publishGate === "passed";
      const outcomeEvidence = released
        ? createOutcomeEvidenceSnapshot({
            ...
            outcome: "released",
            reason:
              "The promoted Agent passed trusted post-release Taskset evaluation.",
      ...
      return nextCreateImproveRunRevision(current, {
        ...
        candidates: updateCandidate(current, candidate.id, {
          ...
          metadata: released
            ? releasedCandidateMetadata(candidate, {
                sourceCommit: profileCommit,
    ...
    if (pullRequest.state === "closed") {
      ...
      return nextCreateImproveRunRevision(current, {
        ...
        blockedReason:
          "The Agent improvement pull request was closed without merge.",
        updatedAt: closedAt,
      });
    }
...
async function verifyAppliedProfileCommit(input: {
  ...
  const command =
    input.command ??
    (async (name, args, cwd, env) => {
      const { runWorkspaceCommand } = await import(
        "../../workspace/workspaces.js"
      );
  ...
  if (!head || head !== input.expectedCommit) {
    throw new Error(
      "The active Profile moved after the change was applied. Review the current Profile before re-running Evals."
    );
  }
  return head;
}
...
function releasedCandidateMetadata(
  ...
  return {
    ...candidate.metadata,
    agentPackageLineage: {
      ...record(candidate.metadata.agentPackageLineage),
      releasedSourceCommit: release.sourceCommit,
      postReleaseReceiptId: release.receiptId,
  ...
  return receipts
    .map((receipt) => {
      ...
      const countText = counts
        ...
        : receipt.status;
      return `- **${receipt.subject === "active" ? "Base" : "Candidate"}** \`${
        receipt.sourceCommit?.slice(0, 12) ?? "unknown"
      }\`: ${countText}; publish gate ${receipt.publishGate}.`;
    })
    .join("\n");
}
