...
export async function executeAgentTasksetEvaluation(input: {
  ...
  if (!tasksetRef)
    throw new Error(
      "Agent Taskset execution requires an approved Taskset ref."
    );
  assertTasksetRefMatches(tasksetRef, input.taskset);
  if (input.taskset.environment.kind !== "agent") {
    throw new Error(
      `Taskset ${input.taskset.id} is not an Agent-shaped Taskset.`
    );
  }
  ...
  if (!tasks.length || tasks.length !== privateRefs.size) {
    throw new Error(
      "The approved Agent Taskset private split could not be resolved exactly."
    );
  }
  ...
  for (const task of tasks) {
    ...
    const parsed =
      result.code === 0 ? parseAgentRunPayload(result.stdout) : null;
    ...
    const attempt = TaskAttemptResultSchema.parse({
      ...
      infrastructureError:
        result.code === 0
          ? null
          : result.stderr.trim().slice(0, 10_000) ||
            `Agent SDK action exited with code ${String(result.code)}.`,
      costUsd: 0,
