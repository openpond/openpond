...
} from "../background-worker-queue.js";
import {
...
export type CreateImproveRuntime = ReturnType<
  typeof createCreateImproveRuntime
>;
...
export function createCreateImproveRuntime(deps: {
  ...
  streamLocalByokChatTurn?: (input: {
    ...
    modelId?: string | null;
    messages: Parameters<
      typeof runModelBackedCreateImprovePlanner
    >[0]["stream"] extends (messages: infer M) => unknown
      ? M
      : never;
    requestId: string;
  ...
  const applyJobs = new KeyedRegistry<BackgroundWorkReceipt>(
    "create/improve apply job"
  );
  ...
  async function applyCreateImproveActionPayload(
    ...
    if (action.runId !== routeRunId) {
      throw new Error(
        "Create/Improve action route does not match the submitted run."
      );
    }
    const mutation = await deps.mutateCreateImproveRun(action, (current) =>
      applyCreateImproveRunAction(current, action)
    );
    ...
    const session = await deps.getSession(
      run.scope.conversationId ?? turn.sessionId
    );
    ...
    if (!mutation.replayed && isCreateImproveMutationState(run.state)) {
      assertCreateImproveMutationApproved({
        actionLabel: "Create/Improve action",
        run,
      });
  ...
  async function resolveCreateImproveApproval(
    ...
    const run =
      turn?.createImproveRun ??
      (approval.providerRequestId
        ? await deps.getCreateImproveRun(String(approval.providerRequestId))
        : null);
    if (!turn || !run)
      throw new Error("Create/Improve approval is missing its run.");
    const type =
      ...
      await deps.appendRuntimeEvent(
        event({
          ...
          status:
            ...
            approval.status === "accepted_for_session"
              ? "completed"
              : "failed",
          output: approval.title,
          data: {
  ...
  function queueAgentImprovementReleaseAction(input: {
    ...
    const receipt = deps.turnFollowUpQueue.enqueue(
      {
        label: `${input.action.type.replaceAll(
          "_",
          " "
      ...
      async () => {
        try {
          const latestRun =
            (await deps.getCreateImproveRun(input.run.id)) ?? input.run;
  ...
  async function planCreateImproveForTurn(input: {
    ...
    const providerId =
      input.turn.modelRef?.providerId ?? input.session.provider;
    const modelId =
    ...
    if (
      ...
      deps.streamLocalByokChatTurn
    ) {
      if (!modelId) {
        throw new Error(
          `Create/Improve planner requires a selected model for provider ${providerId}.`
        );
      }
      return runRecordedPlanner({
        ...input,
    ...
    throw new Error(
      "Create/Improve planner requires OpenPond Chat or a configured OpenAI-compatible provider."
    );
  }
  ...
  async function persistCreateImprovePlanningFailure(input: {
    ...
    const current =
      (await deps.getCreateImproveRun(input.run.id)) ??
  ...
  async function persistCreateImproveRun(input: {
    ...
    assertCreateImproveRunLinked({
      actionLabel: "Persist Create/Improve run",
      run: input.run,
    });
    ...
    const result = await deps.updateTurn(input.turnId, (current) => {
      if (current.sessionId !== input.session.id)
    ...
    await syncCreateImprovePlanApproval({
      session: input.session,
      turn: result,
      run,
    });
  ...
  async function turnForRun(run: CreateImproveRun): Promise<Turn> {
    ...
    if (!turnId)
      throw new Error(`Create/Improve run ${run.id} is not linked to a turn.`);
  ...
  async function setTurnProviderTurnId(
    ...
    await deps.updateTurn(turnId, (current) =>
      current.sessionId === sessionId ? { ...current, providerTurnId } : current
    );
  }
