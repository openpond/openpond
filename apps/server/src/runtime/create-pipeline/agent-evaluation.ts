...
export async function runNormalizedAgentEvaluation(input: {
  ...
  const failed =
    numberValue(payload.summary?.failed) ?? Math.max(0, total - passed);
  const publishGate =
    payload.publishGate?.status === "passed" ? "passed" : "failed";
  ...
  for (const evaluation of results) {
    ...
    if (trace)
      artifactRefs.add(path.posix.join(input.sourceRef, normalizeRef(trace)));
    for (const artifact of stringArray(evaluation.artifacts)) {
      artifactRefs.add(
        path.posix.join(input.sourceRef, normalizeRef(artifact))
      );
    }
  }
  ...
  const evalRefs = [
    ...new Set([...sdkEvalRefs, ...(tasksetExecution?.taskRefs ?? [])]),
  ];
  const combinedPublishGate =
    publishGate === "passed" && (tasksetExecution?.failed ?? 0) === 0
      ? ("passed" as const)
      : ("failed" as const);
  const status =
    result.code === 0 &&
    combinedFailed === 0 &&
    combinedPublishGate === "passed"
      ? "passed"
      : "failed";
  const timestamp = input.timestamp ?? new Date().toISOString();
  return {
    id: `agent_eval_${input.run.id}_${input.subject}_${input.sourceCommit.slice(
      0,
      12
    )}`,
    ...
    summary: `${
      input.subject === "active"
        ...
        ? "Candidate"
        : "Post-release"
    } Agent SDK and Taskset Evals: ${combinedPassed}/${combinedTotal} passed${
      combinedPublishGate === "failed" ? "; publish gate failed" : ""
    }.`,
    createdAt: timestamp,
    metadata: {
...
function requireTaskset(value: Taskset | null | undefined): Taskset {
  if (!value)
    throw new Error(
      "The approved Agent Taskset could not be resolved for evaluation."
    );
  return value;
}

function requireGradeAttempt(
  value: AgentTasksetGrade | undefined
): AgentTasksetGrade {
  if (!value)
    throw new Error(
      "Trusted Taskset grading is unavailable for Agent evaluation."
    );
  return value;
}
