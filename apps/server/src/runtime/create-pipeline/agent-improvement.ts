...
} from "../../agents/agent-package-origin.js";
...
export async function authorAgentImprovementCandidate(
  ...
  try {
    ...
    const taskset = run.tasksetRef
      ...
      : null;
    const authoringContext = taskset
      ? publicTasksetAuthoringContext(run, taskset)
      : null;
    const candidateId = `agent_candidate_${run.id}`;
    const authoringExecutionId = `candidate_authoring_${candidateId}`;
    const evaluationExecutionId = `evaluation_${candidateId}_${
      run.tasksetRef?.revision ?? "sdk"
    }`;
    const existingCandidate = run.candidates.find(
      (candidate) =>
    ...
    const basePackageOrigin = await readAgentPackageOrigin(
      activeTarget.sourceRoot,
      workspace.git.baseCommit
    );
    ...
    if (!existingCandidate) {
      current = nextCreateImproveRunRevision(run, {
        ...
        metadata: {
          ...run.metadata,
          ...(authoringContext
            ? { tasksetAuthoringContext: authoringContext }
            : {}),
          agentImprovement: {
            status: "authoring",
            candidateId,
      ...
      await (
        services.applySource ?? runModelBackedLocalCreateSourceApplication
      )({
        snapshot: current,
    ...
    const committedGit = await (
      services.commitCandidate ?? commitAgentImprovementCandidate
    )({
      run: current,
      activeTarget,
      workspace,
    ...
    return nextCreateImproveRunRevision(current, {
      ...
      evaluationReceipts: [
        ...current.evaluationReceipts.filter(
      ...
      externalExecutionRefs: mergeExecutionRef(current.externalExecutionRefs, {
        ...
        metadata: {
          ...
          executionContractHash:
            activeReceipt.metadata.executionContractHash ?? null,
          completedAt,
        },
      }),
      releaseOutcome: {
    ...
    return nextCreateImproveRunRevision(current, {
      ...
      externalExecutionRefs: current.externalExecutionRefs.map((ref) =>
        (ref.kind === "candidate_authoring" || ref.kind === "evaluation") &&
        ref.status === "running"
          ? {
              ...ref,
              status: "failed",
...
function publicTasksetAuthoringContext(
  ...
  if (!ref)
    throw new Error("Agent authoring requires an approved Taskset ref.");
  ...
  if (tasks.length !== allowed.size) {
    throw new Error(
      "The approved Agent Taskset authoring split could not be resolved exactly."
    );
  }
  return {
    tasksetId: ref.id,
...
function assertMatchedTasksetExecutions(
  ...
  for (const receipt of [active, candidate]) {
    if (
      ...
      throw new Error(
        "Agent comparison did not use the exact approved trusted Taskset execution."
      );
    }
  }
  if (
    typeof active.metadata.executionContractHash !== "string" ||
    active.metadata.executionContractHash !==
      candidate.metadata.executionContractHash
  ) {
    throw new Error(
      "Active and candidate Agent evaluations used different Taskset execution contracts."
    );
  }
}
...
function candidateRecord(input: {
  ...
  basePackageOrigin: AgentPackageOrigin;
  timestamp: string;
}): CreateImproveCandidate {
  return {
    id: input.id,
    ...
    authoringModelRef:
      typeof input.run.metadata.authoringModel === "string"
    ...
    metadata: {
      ...
      agentPackageLineage: {
        baseVersionId: input.basePackageOrigin.versionId,
        basePackageDigest: input.basePackageOrigin.digest,
        validationReceiptIds: input.basePackageOrigin.validationReceiptIds,
        evalReceiptIds: input.basePackageOrigin.evalReceiptIds,
        candidateBaseCommit: input.git.baseCommit,
      },
    },
  };
}
