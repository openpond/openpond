...
import {
  assertLocalCreateSourceLayout,
  runLocalCreatePipelineChecks,
} from "./local-create-pipeline-checks.js";
import type {
...
export { runLocalCreatePipelineChecks } from "./local-create-pipeline-checks.js";
export type {
  LocalCreatePipelineCheckInput,
  LocalCreatePipelineCheckResult,
  LocalCreatePipelineTarget,
...
export const HARNESS_PREPARED_SOURCE_MANIFEST =
  ".openpond-harness-source-application.json";
...
export async function applyApprovedLocalCreateImproveRun(
  ...
  if (
    ...
    return blockLocalCreatePipeline(snapshot, {
      ...
      metadata: {
        ...
        requestedAgentId:
          snapshot.target.kind === "agent" ? snapshot.target.id : null,
      },
    });
  }
  ...
  try {
    const checkResult = await (
      options.runChecks ?? runLocalCreatePipelineChecks
    )({
    ...
    return transitionCreatePipeline(runningChecks, {
      ...
      evaluationReceipts: [
        ...runningChecks.evaluationReceipts.filter(
          (receipt) => receipt.id !== evaluationReceiptId
        ),
        {
...
export function shouldApplyLocalCreatePipeline(
  snapshot: CreateImproveRun
): boolean {
  return Boolean(
...
export function resolveLocalCreatePipelineTarget(
  snapshot: CreateImproveRun
): LocalCreatePipelineTarget {
  const adapter = snapshot.adapter;
  if (adapter.kind !== "local")
    throw new Error("Create/Improve target must use the local adapter.");
  if (snapshot.target.kind !== "agent")
    throw new Error("Local agent source application requires an Agent target.");
  const sourcePath = adapter.sourcePath?.trim();
  const repoPath = adapter.repoPath?.trim();
  if (!sourcePath)
    throw new Error("Local Create approval is missing a profile source path.");
  if (!repoPath)
    throw new Error("Local Create approval is missing a profile repo path.");
  ...
  const sourceRoot =
    snapshot.operation === "improve" && agentId === "default"
      ? resolvedSourcePath
  ...
  return {
    ...
    profileRelativePath: normalizeRelativePath(
      path.relative(resolvedRepoPath, resolvedSourcePath)
    ),
    sourceRoot,
    sourceRootRelativePath: normalizeRelativePath(
      path.relative(resolvedRepoPath, sourceRoot)
    ),
  };
}
...
export async function runModelBackedLocalCreateSourceApplication(input: {
  ...
  ensureCodexRuntime: NonNullable<
    LocalCreatePipelineApplyOptions["ensureCodexRuntime"]
  >;
  appendRuntimeEvent: NonNullable<
    LocalCreatePipelineApplyOptions["appendRuntimeEvent"]
  >;
  setProviderTurnId: NonNullable<
    LocalCreatePipelineApplyOptions["setProviderTurnId"]
  >;
  model: string | null;
  codexReasoningEffort?: CodexTurnInput["codexReasoningEffort"];
}): Promise<void> {
  if (!existsSync(input.target.workspaceRoot)) {
    throw new Error(
      `Profile repo path does not exist: ${input.target.workspaceRoot}`
    );
  }
  if (!existsSync(input.target.sourcePath)) {
    throw new Error(
      `Profile source path does not exist: ${input.target.sourcePath}`
    );
  }
  if (
    await applyPreparedHarnessSourceApplication(input.snapshot, input.target)
  ) {
    await input.appendRuntimeEvent(
      event({
        ...
        status: "completed",
        output: `Applied prepared harness ${
          input.snapshot.operation === "improve" ? "Improve" : "Create"
        } source for ${input.target.agentId}; running local checks.`,
        data: {
  ...
  await input.appendRuntimeEvent(
    event({
      ...
      appId: input.session.appId,
      status: "started",
      output: `Applying approved ${
        input.snapshot.operation === "improve" ? "Improve" : "Create"
      } plan with Codex in ${input.target.sourceRootRelativePath}.`,
      data: {
  ...
  const codexSession: Session = {
    ...
    workspaceId:
      ...
      `openpond-profile:${input.target.activeProfile}`,
    workspaceName:
  ...
  try {
    await runtime.client.waitForTurn(
      providerTurn.turnId,
      LOCAL_CREATE_CODEX_WAIT_TIMEOUT_MS
    );
  } catch (error) {
    if (
      !isCodexTurnTimeoutError(error, providerTurn.turnId) &&
      !isCodexAppServerCleanExitError(error)
    ) {
      throw error;
    }
    ...
    await input.appendRuntimeEvent(
      event({
        ...
        appId: input.session.appId,
        status: "completed",
        output: `Recovered completed ${
          input.snapshot.operation === "improve" ? "Improve" : "Create"
        } source after Codex source application ${recoveryReason} for ${
  ...
  await input.appendRuntimeEvent(
    event({
      ...
      appId: input.session.appId,
      status: "completed",
      output: `Codex completed approved ${
        input.snapshot.operation === "improve" ? "Improve" : "Create"
      } source application for ${input.target.agentId}.`,
      data: {
...
export async function applyPreparedHarnessSourceApplication(
  ...
  const sourcePath = resolveHarnessManifestPath(
    target.workspaceRoot,
    operation.source,
    "source"
  );
  if (!existsSync(sourcePath)) {
    throw new Error(
      `Prepared harness source does not exist: ${operation.source}`
    );
  }
  ...
  for (const registration of operation.registrations ?? []) {
    ...
    if (!existsSync(registrationSource)) {
      throw new Error(
        `Prepared harness registration source does not exist: ${registration.source}`
      );
    }
...
function asHarnessPreparedSourceOperation(
  value: unknown
): HarnessPreparedSourceOperation | null {
  ...
  if (
    ...
    return null;
  if (
    operation.registrations !== undefined &&
    !Array.isArray(operation.registrations)
  ) {
...
function resolveHarnessManifestPath(
  ...
  if (path.isAbsolute(relativePath)) {
    throw new Error(
      `Prepared harness ${label} must be relative to the profile repo.`
    );
  }
  ...
  if (resolved !== root && !resolved.startsWith(`${root}${path.sep}`)) {
    throw new Error(
      `Prepared harness ${label} must stay inside the profile repo.`
    );
  }
  return resolved;
}
...
async function waitForTimedOutSourceApplication(
  target: LocalCreatePipelineTarget
): Promise<boolean> {
...
async function localCreateSourceLayoutIsStable(
  target: LocalCreatePipelineTarget
): Promise<boolean> {
  try {
...
async function sourceLayoutFingerprint(
  target: LocalCreatePipelineTarget
): Promise<string> {
  const paths = [
...
function localCreatePipelinePrompt(
  ...
  return [
    `Apply this already-approved OpenPond ${
      snapshot.operation === "improve" ? "Improve" : "Create"
    } plan by editing the local profile repo source.`,
    "",
...
function sourceActionIdForAgent(actionId: string, agentId: string): string {
  ...
  return actionId.startsWith(prefix)
    ? actionId.slice(prefix.length) || "chat"
    : actionId;
}
