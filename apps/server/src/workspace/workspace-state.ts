...
export async function loadWorkspaceStateAtPath(
  ...
  const [branchResult, branchesResult, statusResult, remoteResult] = await Promise.all([
...
export async function createAndCheckoutBranchAtPath(
  ...
  const state = await loadWorkspaceStateAtPath(paths, workspace, { clone: false });
  ...
  return loadWorkspaceStateAtPath(paths, workspace, { clone: false });
}
