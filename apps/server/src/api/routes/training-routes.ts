...
export async function handleTrainingRoutes({ deps, request, requestUrl, response }: HttpRouteContext): Promise<boolean> {
  ...
  const routes: Array<{ method: string; path: string; action: string; status?: number }> = [
    ...
    { method: "POST", path: "/v1/training/marketing-benchmark/preregister", action: "preregister_marketing_benchmark", status: 201 },
    { method: "POST", path: "/v1/training/marketing-benchmark/runs", action: "run_marketing_benchmark", status: 202 },
    { method: "POST", path: "/v1/training/marketing-benchmark/cancel", action: "cancel_marketing_benchmark" },
