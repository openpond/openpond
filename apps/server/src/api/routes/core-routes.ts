...
export async function handleCoreRoutes({
  ...
  if (request.method === "POST" && requestUrl.pathname === "/v1/profile/evaluations/run-suite") {
    sendJson(response, 200, await profileEvaluationRunSuitePayload(await readJson(request)));
    return true;
  }
  ...
  if (request.method === "POST" && requestUrl.pathname === "/v1/profile/evaluations/reports") {
    sendJson(response, 201, await profileEvaluationSaveReportPayload(await readJson(request)));
    return true;
  }
