...
export function isRecoverableStartupError(error: unknown): error is PersistenceError {
  return error instanceof PersistenceError && /^(INVALID_CONFIG|CONFIG_|MIGRATION_|SOURCE_CHANGED_|CREDENTIAL_|UNSUPPORTED_STORAGE_|INVALID_MIGRATION_|INVALID_ACCOUNT_|MISSING_INSTRUCTION_|INVALID_PROFILE_|DATABASE_RECOVERY_)/.test(error.issue.code);
}
export async function startRecoveryServer(home: string, port: number, initialError: PersistenceError) {
  return ownHomeRuntime(home, async () => {
    const { token, tokenFile } = await ensureCapabilityToken(home);
    ...
    const server = createServer((request, response) => {
      void (async () => {
        ...
        const configEditable = !/^(UNSUPPORTED_STORAGE_|INVALID_MIGRATION_)/.test(initialError.issue.code);
        if (request.method === "GET" && url.pathname === "/v1/recovery") {
          sendJson(response, 200, { issue, configEditable, config: await configuration.status(), migration: await readMigrationJournal(home).catch((error) => ({ issue: persistenceIssue(error, home) })) }); return;
        }
    ...
    await publishRuntimeEndpoint(home, url, "startup-recovery");
    return { url, port: actualPort, token, tokenFile, storePath: storagePaths(home).database, repaired,
      close: async () => { server.closeIdleConnections(); await new Promise<void>((resolve, reject) => server.close((error) => error ? reject(error) : resolve())); } };
  });
}
