...
export function createInitialCreatePipeline(input: CreatePipelineInput): {
  ...
  const plan = CreatePipelinePlanSchema.parse({
    ...
    checks: [
      { name: "inspect", command: "bun run agent:inspect -- --json", required: true },
      { name: "build", command: "bun run agent:build", required: true },
      { name: "validate", command: "bun run agent:validate -- --json", required: true },
      { name: "eval", command: "bun run agent:eval -- --json", required: true },
    ],
    approvalId,
    approvedAt: null,
