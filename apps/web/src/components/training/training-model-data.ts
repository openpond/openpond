...
export function trainingModelRows(
    ...
    .map((taskset) => {
      ...
      const localModel =
        state.models
          .filter(
            (model) =>
              model.tasksetId === taskset.id && model.status === "imported"
          )
          .sort((left, right) =>
            right.importedAt.localeCompare(left.importedAt)
          )[0] ?? null;
      const latestPlan = latestJob
        ? planById.get(latestJob.planId) ?? null
        : plans.sort(newestFirst)[0] ?? null;
...
export function jobsForTaskset(
  state: TrainingStateResponse | null,
  tasksetId: string
): TrainingJob[] {
  if (!state) return [];
...
export function planForJob(
  ...
  return job && state
    ? state.plans.find((plan) => plan.id === job.planId) ?? null
    : null;
}

export function artifactsForJob(
  state: TrainingStateResponse | null,
  jobId: string | null
): TrainingArtifact[] {
  return jobId && state
    ? state.artifacts.filter((artifact) => artifact.jobId === jobId)
    : [];
}

export function tasksetMethod(taskset: Taskset): string {
  const authoredMethod = taskset.metadata.trainingMethod;
  if (typeof authoredMethod === "string" && authoredMethod !== "none")
    return authoredMethod;
  if (
    taskset.readiness?.recommendedMethod &&
    taskset.readiness.recommendedMethod !== "none"
  )
    return taskset.readiness.recommendedMethod;
  return (
    taskset.capabilities.compatibleMethods.find(
      (method) => !["none", "retrieval"].includes(method)
    ) ?? "sft"
  );
}

export function trainingRunMethodLabel(
  taskset: Taskset,
...
export function modelName(value: string, _method: string): string {
  const name = /\b(model|adapter)\s*$/i.test(value.trim())
    ? value.trim()
    : `${value.trim()} model`;
  return name;
}
