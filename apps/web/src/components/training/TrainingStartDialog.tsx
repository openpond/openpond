...
export function TrainingStartDialog({
  ...
  const [sequenceLength, setSequenceLength] = useState(() => recommendedSequenceLength(taskset));
  ...
  async function start() {
    ...
    const completed = await onStart(destinationId, sftRecipe({ maxSteps, sequenceLength, rank, learningRate, model: selectedModel }));
  ...
  return <div className="training-dialog-backdrop" role="presentation" onMouseDown={busy ? undefined : onClose}>
    <section className="training-dialog training-start-dialog" role="dialog" aria-modal="true" aria-label="Start training" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="training-start-fields">
        ...
        <label><span>Base model</span><select value={baseModelId} disabled={busy} onChange={(event) => { const modelId = event.target.value; setBaseModelId(modelId); setLearningRate(defaultLearningRate(modelId)); }}><option value="openpond/tiny-cpu-gpt2-fixture">Tiny CPU correctness fixture</option>{trainableModels.map((model) => <option key={model.id} value={model.modelId ?? model.id}>{model.name} · {formatBytes(model.sizeBytes)}</option>)}</select></label>
      ...
      <dl className="training-start-summary">
        ...
        <div><dt>Estimate</dt><dd>{destinationId === "local_cpu_fixture" ? selectedModel ? `$0 · ${maxSteps} steps × ${sequenceLength} tokens · 15-minute hard stop` : "$0 · 2-minute hard stop" : "Provided before approval"}</dd></div>
...
function sftRecipe(input: { maxSteps: number; sequenceLength: number; rank: number; learningRate: number; model: ModelAsset | null }): SftRecipe {
