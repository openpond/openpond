...
export function TaskDraftTasks({ draft, disabled, onChange, selectedId, onSelect: setSelectedId }: {
  draft: TasksetDraft; disabled: boolean; onChange: (draft: TasksetDraft) => void;
  ...
  if (selected) return <EditorSection title="Edit task" description="Changes stay in this draft until you save or publish the collection.">
      <TaskTextField autoFocus label="Input" value={selected.input} disabled={disabled} onChange={input => update({ ...selected, input })} />
      ...
      <footer className="model-build-actions"><button className="training-text-button danger" type="button" disabled={disabled} onClick={() => remove(selected)}>Remove task</button><button className="training-button" type="button" onClick={() => setSelectedId(null)}>Done</button></footer>
  </EditorSection>;
...
function TaskTextField({ label, value, defaultKey = "prompt", disabled, onChange, autoFocus = false }: { label: string; value: Record<string, unknown>; defaultKey?: string; disabled: boolean; onChange: (value: Record<string, unknown>) => void; autoFocus?: boolean }) {
  const key = textKey(value) ?? (Object.keys(value).length === 0 ? defaultKey : null);
  return <div>{key ? <Field label={label}><textarea autoFocus={autoFocus} rows={5} disabled={disabled} value={typeof value[key] === "string" ? value[key] : ""} onChange={event => onChange({ ...value, [key]: event.target.value })} /></Field> : <p>{label} uses structured fields.</p>}
