...
function TasksetRewardPreview({ client, taskset }: { client: OpenPondLearningClient | null; taskset: Taskset }) {
  ...
  return <section className="learning-workspace">
    ...
    <p>These graders belong to the selected Taskset release. Changing them requires publishing a new Taskset release; existing models keep their selected configuration.</p>
    {binding.success ? <RewardBindingSummary client={client} binding={binding.data} /> : taskset.graders.length ? taskset.graders.map((grader) => <details key={grader.id}><summary>{grader.label} · {grader.kind === "model_judge" ? "LLM judge" : grader.kind === "human" ? "Human review" : "Code verifier"}</summary><p>{grader.rewardEligible ? "Training grader" : "Evaluation only"} · weight {grader.weight}{grader.hardGate ? " · hard gate" : ""}</p>{grader.kind === "model_judge" ? <><p>{grader.judge.providerId} / {grader.judge.modelId} · calibration {grader.calibrationStatus}</p><pre>{grader.rubric}</pre></> : grader.kind === "human" ? <pre>{grader.rubric}</pre> : grader.kind === "custom_verifier" ? <p>Source: {grader.module} · export {grader.exportName} · {grader.timeoutMs} ms limit</p> : <pre>{JSON.stringify(grader.config, null, 2)}</pre>}</details>) : <p>No graders are configured in this release. Add quality checks in Tasksets before reward-based training.</p>}
...
function StarterReward({ starter }: { starter: ModelStarterPreview }) {
  ...
  const type = kinds.length > 1 ? "Grader combination" : kinds[0] ?? "Code verifier";
  return <section className="learning-workspace model-creation-reward">
    <label>Grader type<select aria-label="Grader type" disabled value={type}><option value={type}>{type}</option></select></label>
    <label>Grader<select aria-label="Grader" disabled value={starter.reward.name}><option value={starter.reward.name}>{starter.reward.name}</option></select></label>
