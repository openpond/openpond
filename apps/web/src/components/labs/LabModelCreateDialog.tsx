...
export function LabModelCreateDialog({ baseModelCandidates, tasksets, busy: saving, initialName, project = null, starter = null, learningClient, onClose, onCheck, onCreate, onSaved, onManageModels, renderTasksetBuilder }: {
  ...
  function configuration(): LabModelCreateInput {
    const starterRequest = pendingRequest ?? (activeStarter && draft.baseModel ? { schemaVersion: "openpond.modelStarterCreation.v1" as const, operationId, profileId: activeStarter.profileId, modelId: id, name: normalizedName, starter: { id: activeStarter.preview.starter.id, revision: activeStarter.preview.starter.revision, contentHash: activeStarter.preview.starter.contentHash }, startingModel: draft.baseModel, method: activeStarter.preview.starter.defaultMethod } : undefined);
      ...
      <form onSubmit={(event) => { void submit(event); }}>
        <fieldset disabled={fieldsLocked} className="model-create-fields">
          {step === 0 ? <>
            ...
            {activeStarter ? <section className="model-creation-reward"><h3>{activeStarter.preview.reward.name}</h3><p>{activeStarter.preview.reward.description}</p>{activeStarter.preview.reward.checks.map((check, index) => <div key={index}><h4>{check.name}</h4><p>{check.description}</p><p>{check.kind === "custom_verifier" ? "Code verifier" : check.kind === "model_judge" ? "LLM judge" : check.kind === "human" ? "Human review" : "Deterministic check"}{check.hardGate ? " · Required to pass" : ""}</p></div>)}<p>These published checks are imported with the tasks. You can edit them in the model’s Rewards after creation.</p></section> : <ModelCreationReward client={learningClient} taskset={selectedTaskset} bindingRef={draft.rewardBindingRef} onChange={(rewardBindingRef) => patch({ rewardBindingRef })} />}
        ...
        <footer><button disabled={busy} type="button" onClick={() => { void guard.requestLeave(onClose); }}>Cancel</button>{step > 0 ? <button disabled={fieldsLocked} type="button" onClick={() => { setStep(step - 1); setError(null); }}>Back</button> : null}<button disabled={busy || !normalizedName} type="submit">{checking ? "Checking…" : saving ? "Saving…" : pendingRequest ? "Retry creation" : step === 1 && !selectedTaskset && !activeStarter ? "Skip" : step < 2 ? "Continue" : project ? "Save changes" : "Create model"}</button></footer>
      </form>
    </AppDialog>
