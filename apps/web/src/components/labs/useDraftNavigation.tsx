...
export function useDraftNavigation(input: { dirty: boolean; busy?: boolean; name: string; save: () => Promise<boolean>; onLeave?: () => void }) {
  ...
  const dialog = open ? <AppDialog ariaLabel={`Save ${input.name} before leaving`} className="labs-rename-dialog" backdropClassName="labs-rename-backdrop" dismissDisabled={saving} onClose={() => resolve(false)}>
    <h2>Save {input.name} before leaving?</h2>
    <p>Your changes can be saved before opening the next page.</p>
    ...
    <div className="model-build-actions">
      ...
      <button className="training-button" type="button" disabled={saving} onClick={async () => {
        setSaving(true);
