...
export function TaskFormatEditor(props: { client: OpenPondLearningClient | null; definition: TaskDefinition | null; onClose: () => void; onSaved: (definition: TaskDefinition) => void }) {
...
function TaskFormatEditorForm({ client, definition, binding, onClose, onSaved }: { client: OpenPondLearningClient | null; definition: TaskDefinition | null; binding: RewardBinding | null; onClose: () => void; onSaved: (definition: TaskDefinition) => void }) {
  const [id] = useState(() => definition?.id ?? `format-${crypto.randomUUID()}`);
  ...
  async function save() {
    const result = await mutation.run(async (api) => {
      ...
      const response = await api.command({ action: "publish_resources", operationId: `format:${release.contentHash}`, resources: [{ kind: "binding", expectedRevision: 0, content: bindingContent }, { kind: "definition", expectedRevision: revision, content }, { kind: "source", expectedRevision: 0, content: source }] });
