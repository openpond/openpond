...
export function LearningCombinedRewardsPage({ client, selectedId, after, onSelect, onPage }: { client: OpenPondLearningClient | null; selectedId: string | null; after: string | null; onSelect: (id: string | null) => void; onPage: (after: string | null) => void }) {
  ...
  return <div className="labs-flat-body labs-resource-page learning-workspace">
    ...
    {entry ? <>
      ...
      <AuthoringDraftList client={client} targetKind="binding" onResume={setResuming} />
    ...
    {resuming ? <AppDialog ariaLabel="Resume Combined Reward" className="labs-rename-dialog labs-model-taskset-dialog learning-editor-dialog" backdropClassName="labs-rename-backdrop" dismissDisabled onClose={() => undefined}><AuthoringDraftEditor key={resuming.id} client={client} draft={resuming} onClose={() => setResuming(null)} onPublished={id => { setResuming(null); selected.refresh(); resources.refresh(); onSelect(id); }} /></AppDialog> : null}
    {editing ? <AppDialog ariaLabel="Combined Reward" className="labs-rename-dialog labs-model-taskset-dialog learning-editor-dialog" backdropClassName="labs-rename-backdrop" dismissDisabled onClose={() => undefined}><CombinedRewardEditor key={editing === "new" ? "new" : `${editing.id}:${editing.revision}`} client={client} binding={editing === "new" ? null : editing} onSaved={(binding) => { setEditing(null); selected.refresh(); resources.refresh(); onSelect(binding.id); }} onClose={() => setEditing(null)} /></AppDialog> : null}
  </div>;
}
