...
export function useRightChatPanelViews(input: {
  ...
  return useMemo(() => {
    ...
    return rightChatPanels.map((panel) => {
      ...
      const panelPendingApproval = latestPendingApprovalForSession(
        panelIndexes,
        panel.sessionId
      );
      ...
      const createImproveRun = latestCreateImproveRunProjection({
        events: panelEvents,
      });
      ...
      return {
        ...
        steerAutoDispatchBlocked:
          Boolean(panelPendingApproval) ||
          panelTurnCompletionState === "blocked",
        steerAutoDispatchReady:
          panelTurnCompletionState === "completed" &&
          !panelPendingApproval &&
          !panelRunning,
        workspaceRootPath,
